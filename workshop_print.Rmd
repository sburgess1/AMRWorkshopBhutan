...
```{r, eval = FALSE}
ecoli %>%
  select(`Dzongkhag (sample source)`, `Tetracyclin (30µg)`) %>%
  filter(`Tetracyclin (30µg)` < 15)

#select and filter using column index
ecoli %>%
  select(`Dzongkhag (sample source)`, 29) %>%
  filter(.[[2]] < 15)
```
...
```{r, results = FALSE}
#Rename columns
colnames(ecoli_select) <- c("meatshop", "stratification", "date_collection", "sample_type", "sample_collection", "sample_weight_Kg", "sample_source", "farm_category", "slaughter", "transportation", "transport_medium", "transport_other_meat", "GEN", "CHL", "MEM", "CRO", "CIP",  "FEP", "NAL", "AMP", "TCY", "SXT")
...
Open an R script and save it as workshop.R. Write an R script to read in and tidy the E. coli sheet from the version 2 excel file. It should contain the following:

1. Reads in the E.coli sheet and assigns it to ecoli.

2. Chains together the following operations and assigns it to ecoli_select.
...
3. Changes the date column to date format (using the lubridate library). Use ?lubridate to check what function should be used for parsing a date in the year-month-day format.
...
Check whether the categorical variable values are consistent for the meat_shop categorical variable using the table function.Add these to your R script.
...
Add code for the following to your R script.
...
salmonella_select <- salmonella%>% 
  select(!starts_with("SIR"), -c(1, 15, 34:35)) %>%
...
df_join <- full_join(ecoli, meta, by = "Stratification")
```
...
If the ecoli, enterococci and salmonella data sets are not still in your environment. Reload, tidy them and join them together by rerunning the script generated from day 1.
...
Add the above code to your R script.
...
1. Generate SIR columns for the new df. First change the disk diffusion values to a disk data class and assign this to df2. Next, copy the disk diffusion values to new columns and covert the original disk diffusion columns to SIR and assign this to df3. Copy this code into your workshop.R script.
...
4. Use dplyr to generate an antibiogram of your choice and ggplot (using the ggplot_sir() function) to generate a stacked bar plot for one antimicrobial grouped by sample_collection.
...
Antibiogram for the antibiotics CHL, AMP and TCY:
...
Antibiogram for the species *E. coli*:
...
Antibiogram for *E. coli* using the antibiotics "CHL", "AMP", "TCY" and split into the groups fridge and display table  (from the variable sample_collection). 
...
If the merged ecoli, enterococci and salmonella data set is not still in your environment. Reload the data sets, tidy them and join them together by rerunning the script generated from day 1 and 2 (to generate the data frame assigned to df3).
...
our_data %>%
  mutate(first = first_isolate(method = "episode-based", episode_days = 7))#7day interval from initial isolate
our_data %>% #Specify antimicrobial used to identify first isolate
...
The group/class that each antimicrobial can be checked by filtering the antibiotics data set from the AMR R package.
...
PCA analysis can be used for grouping and visualising data sets. Note when using the AMR R package for PCA analysis 30 observations are required for each grouped variable.
...
If the merged ecoli, enterococci and salmonella data set and the ecoli3 dataset are not still in your environment. Reload the data sets, tidy them and join them together by rerunning the script generated from days 1 and 2 to generate the df3 and ecoli3 data frames.
...
`geom<name>(mapping = aes(x = <x_variable>, y = <y_variable>, color = <variable_for_color>)) + labs(x = <x_axis_label>, y = <y_axis_label>))`
...
This example uses the ecoli data set to generate a line graph to show the percentage of resistant *E. coli* over time. First the data is prepared.
...
This example below illustrates how to generate a map using the rnaturalearth package and overlay it with prevalence data. 
...
ggplot(data = map_data) +
  ...
  geom_sf_text(aes(label = ifelse(is.na(percent_resistant), name_sv,  # if NA label with name_sv (region) 
                                  paste0(name_sv, "\n", round(percent_resistant, 1), "%")  # else label with name_sv and percent_resistant
                                  )), color = "black", size = 3) +  # Colors region names black size 3
  scale_fill_gradient(low = "lightblue", high = "darkblue", na.value = "white") +  # <- Closing parenthesis added here
...
Try removing some parts of the code above to see how the plot changes.

***Multi-panel boxplot***

The example below uses the ecoli dataset to illustrate how to generate a multi-panel boxplot for two collection points.
...
ggplot(df_summary, aes(x = antibiotic, y = mean_resistance, fill = antibiotic)) +
  geom_boxplot(alpha = 0.5) +  # Boxplot (removing default outliers)
...
The example below demonstrates how to generate a mirrored barplot similar to that in presented in Mughini-Gras 2019 Lancet Planet Health 2019: 
3: 357–69. 
...
# Create the mirrored bar chart  with no colour gradient
ggplot(data) +
  ...
  labs(x = "ESBL prevalence (%)       |       Human exposure (%)",
       y = "") +
  theme_minimal()
#Create the mirrored bar chart with a colour gradient
ggplot(data) +
...
The example below illustrates how to generate an upset plot. Upset plots are used to visualise intersections of data sets (e.g. as an alternative to Venn diagrams). For further information on upset plots see https://jokergoo.github.io/ComplexHeatmap-reference/book/upset-plot.html.
