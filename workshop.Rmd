...
ecoli %>%
  select(`Dzongkhag (sample source)`, `Sample ID`, colnames(ecoli)[31]) %>%
  filter(get(colnames(ecoli)[31]) < 15)
...
1. Re-read version 2 of the excel file, which has the correct date format and assign it to ecoli.
...
The number of MDR isolates can be determined using the mdro() function. Use the example_isolates dataset to determine the number of MDR isolates. Note it is unclear based on the tutorial how it is determining whether an isolate is MDR or not. It is not based on 3 classes or more. 

```{r}
#Generate a table with the total number of MDR isolates
mdr <- mdro(example_isolates, guideline = "EUCAST", pct_required_classes = 0.5)
table(mdr)
...
example_isolates %>%
  ...
  filter(MDRO == "Positive") #Filters for those rows that are MDR positive
```
...
mdr <- mdro(df3, guideline = "EUCAST", pct_required_classes = 0.5)
df3 %>%
  mutate(MDRO = mdr) %>%
  group_by(species) %>%  #l
  count(MDRO)
...
The data is now in format to be plotted.
...
#Reshape data example using unite
ecoli_long <- ecoli_res %>%
  pivot_longer(cols = AMP:TCY, names_to = "antibiotic", values_to = "resistance") %>% #Reshape date into long format where antibiotics becomes a single column
  mutate(resistance = ifelse(as.numeric(resistance) == 3, 1, 0)) %>% #Modify resistance column if level 3 (for R) set to 1, otherwise 0
