...
setNames(ecoli, c("meat_shop", "stratification", "date_collection", "sample_type",
...
ec_test <- ecoli_select %>% mutate(species = "Escherichia coli")
str(ec_test)
...
Use str_replace to remove the "." and replace "Bebak MS with BK MS" and assign to the object ecoli_select (add this to your workshop.R script).
...
Check whether any of the other categorical values need to be changed and change where appropriate.Add these to your R script. 
...
hist(ecoli_select$GEN, breaks = 20)
...
```{r, results='hide', echo = FALSE, mesage = FALSE}
...
If the merged ecoli, enterococci and salmonella data set is not still in your environment. Reload the data sets, tidy them and join them together by rerunning the script generated from day 1 (to generate the data frame assigned to df3).
...
The example below illustrates how to generate a map using the rnaturalearth package and overlay it with prevalence data. 

First reshape the data

```{r, results = 'hide', eval = FALSE}
#Reshape data example using unite, example for me only.
ecoli_long <- ecoli_res %>%
  pivot_longer(cols = AMP:TCY, names_to = "antibiotic", values_to = "resistance") %>% #Reshape date into long format where antibiotics becomes a single column
  mutate(resistance = ifelse(as.numeric(resistance) == 3, 1, 0)) %>% #Modify resistance column if level 3 (for R) set to 1, otherwise 0
  unite(source_location, sample_source, location_name, sep = "_", remove = FALSE) #Generate new column source_location by uniting the sample-source and location_name columns
```
```{r}
...
dat$Region2 <- factor(dat$Region2)
str(dat)

#Create a colour
myPalette <- palette(c( "#1F77B4", "#2CA02C", "#9467BD", "#E377C2", "#7F7F7F")) #set colours to use
...
The code below was adapted by Patrick Biggs and Natalia Gonzalez from https://stackoverflow.com/questions/20872133/using-stat-binhex-with-ggpairs 
