---
title: "R tutorial for analysising AMR phenotypic data"
author: "Sara Burgess"
date: "`r Sys.Date()`"
...
dat <- dat %>%
  mutate(across(CTX:STR, as.disk))%>% # convert values to data class "disk"
  mutate(STR = case_when(
    STR >= 15 ~ "S",  # Replace with your own breakpoints
    STR < 12 ~ "R",
    TRUE ~ "I"       # Intermediate
  )) %>%
  mutate_if(is.disk, as.sir, guideline = "CLSI")  #Convert disk columns to SIR class 
...
ant <- antibiogram(ecoli3,
            antibiotics = c("GEN", "CHL", "MEM", "CRO", "CIP", #select antibiotics to include in antibiogram
                           "FEP", "NAL", "AMP", "TCY", "SXT"), 
            ab_transform = "name", #transform antibioitc abbreviations into full names
...
The AMR R package cannot be used to generate an MIC frequency table so here dply has been used to generate this table. 
...
```{r}
x <- mdro(example_isolates, guideline = "EUCAST", pct_required_classes = 0.5)
table(x)
...
example_isolates %>%
  mutate(MDRO = x) %>%
  group_by(mo) %>%  # Use mo_species if you want species-level
  count(MDRO)
...
y <- mdro(df3, guideline = "EUCAST", pct_required_classes = 0.1)
df3 %>%
  mutate(MDRO = y) %>%
  group_by(species) %>%  # Use mo_species if you want species-level
  count(MDRO)
...
Note 30 observations are required for each grouped variable.
...
In the above example by adding in the smooth geom it adds in an overall trend line  as well as the 95% confidence intervals for that line, which are represented by the gray zone.
...
ggplot(data = df4) + 
  geom_bar(mapping = aes(x = sample_collection, fill = CHL)) +
  coord_flip() +
  labs(x = "Isolate count", y = "Sample collection point")
...
Use theme() function for things that relate to the entire plot e.g. background colour. 

alpha changes the transparency of the colour. 

Assigning colour to a constant colour versus assinging it to a variable. If assigning it to a variable it has to wrapped within the aes function. 
